// Edge Function: receive-at-collection
// Collection point staff marks a package as received and sends "Ready for Collection" email
// Deno runtime for Supabase Edge Functions

import { serve } from 'https://deno.land/std@0.177.0/http/server.ts'
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2.45.0'

interface ReceiveAtCollectionRequest {
  package_id: string
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Check if calling user is collection staff or admin
    ...
    if (!['collection', 'admin'].includes(callerProfile.role)) {
      return new Response(
        JSON.stringify({
          error: 'Only collection point staff and admins can receive packages',
    ...
    // Parse request body
    const body: ReceiveAtCollectionRequest = await req.json()
    ...
    // Get current package state with delivery location and items
    ...
    // Validate package status - must be in_transit
    if (pkg.status !== 'in_transit') {
      return new Response(
        JSON.stringify({
          error: 'Invalid package status for receiving',
          details: `Package ${pkg.reference} has status '${pkg.status}'. Only packages in transit can be received at collection point.`
        }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    // Update package status to ready_for_collection
    const { data: updatedPackage, error: updateError } = await adminClient
      .from('packages')
      .update({
        status: 'ready_for_collection',
        received_by: callingUser.id,
        received_at: now,
        updated_at: now
      })
      .eq('id', package_id)
    ...
    // Send "Ready for Collection" email notification
    let emailSent = false
      ...
      if (resendApiKey) {
        const emailResponse = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            ...
            subject: `✅ Ready for Collection - ${pkg.reference}`,
            html: `
              <!DOCTYPE html>
              <html>
              <head>
              ...
              <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #374151;">
                <div style="background: #059669; padding: 20px; border-radius: 8px 8px 0 0; text-align: center;">
                  <h1 style="color: white; margin: 0; font-size: 24px;">✅ Ready for Collection!</h1>
                </div>
                
                <div style="background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none;">
                  <p style="margin: 0 0 20px 0;">Hello,</p>
                  <p style="margin: 0 0 20px 0;"><strong>Your package has arrived and is ready for collection!</strong> Please visit the collection point at your earliest convenience.</p>
                  ...
                  <div style="background: #ecfdf5; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #10b981;">
                    ...
                    <p style="font-size: 28px; font-weight: bold; color: #065f46; margin: 0; font-family: monospace;">${pkg.reference}</p>
                  </div>
                  ...
                  <div style="background: #d1fae5; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #059669;">
                    <h3 style="color: #065f46; font-size: 16px; margin: 0 0 10px 0;">📍 Delivery Complete!</h3>
                    <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                      <div style="background: #10b981; width: 12px; height: 12px; border-radius: 50%;"></div>
                      <span style="font-size: 14px; color: #065f46;">Package prepared</span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                      <div style="background: #10b981; width: 12px; height: 12px; border-radius: 50%;"></div>
                      <span style="font-size: 14px; color: #065f46;">Package picked up by driver</span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 10px;">
                      <div style="background: #10b981; width: 12px; height: 12px; border-radius: 50%;"></div>
                      <span style="font-size: 14px; color: #065f46;"><strong>Arrived at collection point - Ready for pickup!</strong></span>
                    </div>
                  </div>
                  
                  <h2 style="color: #1e3a5f; font-size: 18px; margin: 30px 0 15px 0;">📍 Collection Details</h2>
                  
                  <div style="background: #fef3c7; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #f59e0b;">
                    <p style="margin: 0 0 8px 0; font-size: 12px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">Collection Location</p>
                    <p style="font-size: 18px; font-weight: bold; color: #92400e; margin: 0;">${locationName}</p>
                    ${locationAddress ? `<p style="font-size: 14px; color: #b45309; margin: 8px 0 0 0;">${locationAddress}</p>` : ''}
                  ...
                  <div style="background: #f3f4f6; padding: 15px; border-radius: 8px; margin: 25px 0;">
                    ...
                    <ul style="margin: 10px 0 0 0; padding-left: 20px; font-size: 14px;">
                      <li>Your package reference number: <strong>${pkg.reference}</strong></li>
    ...
    await adminClient.from('audit_logs').insert({
      action: 'PACKAGE_RECEIVED_AT_COLLECTION',
      entity_type: 'package',
      ...
      metadata: {
        ...
        received_by_name: callerProfile.full_name,
        received_by_id: callerProfile.id,
        received_at: now,
    ...
    // Also log the ready for collection notification if email was sent
    if (emailSent) {
      await adminClient.from('audit_logs').insert({
        action: 'READY_FOR_COLLECTION_NOTIFICATION',
        entity_type: 'package',
        entity_id: package_id,
        performed_by: callingUser.id,
        metadata: {
    ...
    return new Response(
      JSON.stringify({
        ...
        email_sent: emailSent,
        email_error: emailError,
        message: `Package ${pkg.reference} is now ready for collection`
      }),
